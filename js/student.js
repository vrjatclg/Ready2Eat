...
let menu = [];
let currentPid = null; // Current student PID, not stored locally
...
async function addToCart(item){
  if (!currentPid) {
    // If no PID set, prompt for it first
    await promptForPid();
    if (!currentPid) return; // User cancelled
  }
...
async function promptForPid() {
  return new Promise((resolve) => {
    ...
    const onSubmit = async (e) => {
      ...
      if (pid.length < 3) { 
        pidWarnings.textContent = "Please enter a valid PID"; 
        return; 
      }
      
      currentPid = pid;
      await restoreCart(); // Load existing cart for this PID
      pidDialog.close();
      pidForm.removeEventListener("submit", onSubmit);
      resolve(pid);
    };
    
    pidForm.addEventListener("submit", onSubmit);
  });
}
...
async function onViewOrdersSubmit(e){
  e.preventDefault();
  const pid = (ordersPidInput.value || "").trim().toUpperCase();
  if (!pid) return;
  await renderOrdersList(pid);
}
